...
static void handle_battery(BatteryChargeState charge_state) {
    ...
    snprintf(s_buffer, sizeof(s_buffer), "[leo@pebble:~]$ info\nbattery:   %s", battery_text);
...
static void update_time() {
    ...
    snprintf(s_buffer, sizeof(s_buffer), "[leo@pebble:~]$ date\n%s",time_text);
...
static void main_window_load(Window *window) {
    ...
    //Add custom font
    s_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_HACK_12));	

    //Time layer
    s_time_layer = text_layer_create(GRect(2, 30, bounds.size.w, 40));
    ...
    //Battery layer
    s_battery_layer = text_layer_create(GRect(2, 70, bounds.size.w, 50));
    ...
    //Connection layer
    s_connection_layer = text_layer_create(GRect(2, 93, bounds.size.w, 20));
